...
This repository contains Python scripts to **automate the retrieval, cleaning, and analysis of biomedical articles**. The pipeline helps you convert lists of PubMed IDs into structured data about drugs, their concentrations, and experimental effects on cell lines. Features: Automatically maps PubMed IDs (PMIDs) to PubMed Central IDs (PMCIDs); Downloads and extracts full-text articles in XML or PDF format; Cleans and organizes text into readable files; Searches text for drug mentions using fuzzy matching; Extracts structured efficacy data (drug, concentration, effect, cell line) using AI (ChatGPT API). As an example, a list of pubmed IDs is provided. **One of the main challenges is that only a limited number of papers in this category are available through PubMed Central**.   
